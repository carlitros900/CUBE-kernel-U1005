...
#MID_ShiYouHui add for custom codegen.dws @2015-10-08 >>>
ifneq ($(strip $(MID_PROJECT)),)
	MID_DWS_FILE := $(srctree)/../device/mediateksample/$(MTK_PROJECT)/mid/$(MID_PROJECT)/codegen.dws
$(info kernel mid dct file = $(MID_DWS_FILE))
	ifeq ($(MID_DWS_FILE), $(wildcard $(MID_DWS_FILE)))
	    DWS_FILE := $(MID_DWS_FILE)
	else
DWS_FILE := $(srctree)/$(DRVGEN_PATH)/$(MTK_PROJECT).dws
	endif
else
...
$(DRVGEN_FILE_LIST): $(DRVGEN_TOOL) $(DWS_FILE) $(DTS_FILES)
	for i in $(DTS_FILES); do \
		...
		if [ -f $$dws_path ] ; then \
			mkdir -p $$prj_path ;\
			$(python) $(DRVGEN_TOOL) $(DWS_FILE) $$prj_path $$prj_path cust_dtsi;\
		fi \
	done
ifeq ($(strip $(CONFIG_MTK_DTBO_FEATURE)), y)
