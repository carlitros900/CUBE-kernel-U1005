...
unsigned int ak4376_power_en;
//unsigned int akm4376_pnd_gpio;
...
static const struct reg_default ak4376_reg[] = {
	...
	{ 0x8, 0x11 },	/*	0x08	AK4376_08_RESERVED				*/
	{ 0x9, 0x00 },	/*	0x09	AK4376_09_RESERVED				*/
	{ 0xA, 0x00 },	/*	0x0A	AK4376_0A_RESERVED				*/
	{ 0xB, 0x11 },	/*	0x0B	AK4376_0B_LCH_OUTPUT_VOLUME		*/
	{ 0xC, 0x11 },	/*	0x0C	AK4376_0C_RCH_OUTPUT_VOLUME		*/
...
unsigned int ak4376_i2c_read(struct snd_soc_codec *codec, unsigned int reg)
{
  ...
  //  static  unsigned char  ak_addr = 10 ;
...
//	xfer[0].addr = ak_addr;
//	akdbgprt("\t[ak4376] hong %s  ak_addr = %d \n", __FUNCTION__, ak_addr );
//	ak_addr++ ;
	xfer[0].flags = 0;
	xfer[0].len = 1;
	xfer[0].buf = tx;
...
static int ak4376_parse_dt(struct ak4376_priv *ak4376)
{
	...
	printk("Read akm4376_pnd_gpio  ak4376_power_en pin from device tree\n");
	ak4376_power_en = of_get_named_gpio(dev->of_node, "ak4376_power_en", 0);
	//printk("xuchen ak4376_power_en=%d\n",ak4376_power_en);
	//gpio_free(ak4376_power_en);
	gpio_request(ak4376_power_en, "ak4376_power_en");
	gpio_direction_output(ak4376_power_en, 1);
	printk("\t[AK4376] ak4376_power_en = 0x%x\n", ak4376_power_en);


	ak4376->priv_pdn_en = of_get_named_gpio(dev->of_node, "akm4376_pnd_gpio", 0);
	//printk("xuchen priv_pdn_en=%d\n",ak4376->priv_pdn_en);
	gpio_free(ak4376->priv_pdn_en);
	ret =  gpio_request(ak4376->priv_pdn_en, "akm4376_pnd_gpio");
	akdbgprt("\t[AK4376] %s : gpio_request ret = %d\n",__FUNCTION__, ret);
	if (ret) {
		akdbgprt("\t[AK4376] %s(%d) cannot get ak4376 pdn gpio\n",__FUNCTION__,__LINE__);
		ak4376->pdn1 = 2;	//No use GPIO control
	}
	//ak4376->priv_pdn_en = 132;//force  gpio for 60
	//ak4376->pdn1 = 2;	//No use GPIO control
...
static int ak4376_probe(struct snd_soc_codec *codec)
{
	...
	akdbgprt("\t[AK4376] ak4376_parse_dt %s(%d) ret=%d\n",__FUNCTION__,__LINE__,ret);
...
static struct i2c_driver ak4376_i2c_driver = {
	.driver = {
		.name = "ak4376",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(ak4376_i2c_dt_ids),	//3
	},
