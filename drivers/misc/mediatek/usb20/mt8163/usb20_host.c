...
/*otg_usb_en*/
#ifdef CONFIG_OTG_USB_EN
struct pinctrl_state *pinctrl_otgusben;
struct pinctrl_state *pinctrl_otgusben_low;
struct pinctrl_state *pinctrl_otgusben_high;
#endif
...
/*otg_usb_en*/
#ifdef CONFIG_OTG_USB_EN
void mt_otg_usb_en(void)
{
	int ret = 0;
	printk("****%s:%d before Init OTG_USB_EN KS!!!!!\n", __func__, __LINE__);
	pinctrl_otgusben = pinctrl_lookup_state(pinctrl, "otgusden_init");
	if (IS_ERR(pinctrl_otgusben)) {
		ret = PTR_ERR(pinctrl_otgusben);
		dev_err(mtk_musb->controller, "Cannot find usb pinctrl otgusben\n");
	}

	pinctrl_otgusben_low = pinctrl_lookup_state(pinctrl, "otgusben_low");
	if (IS_ERR(pinctrl_otgusben_low)) {
		ret = PTR_ERR(pinctrl_otgusben_low);
		dev_err(mtk_musb->controller, "Cannot find usb pinctrl otgusben_low\n");
	}

	pinctrl_otgusben_high = pinctrl_lookup_state(pinctrl, "otgusben_high");
	if (IS_ERR(pinctrl_otgusben_high)) {
		ret = PTR_ERR(pinctrl_otgusben_high);
		dev_err(mtk_musb->controller, "Cannot find usb pinctrl otgusben_high\n");
	}
...
static bool musb_is_host(void)
{
	...
	if (devctl & MUSB_DEVCTL_BDEVICE || iddig_state) {
		...
		/*otg_usb_en*/
		#ifdef CONFIG_OTG_USB_EN
		pinctrl_select_state(pinctrl, pinctrl_otgusben_low);
		#endif
	} else {
		usb_is_host = true;
		...
		/*otg_usb_en*/
		#ifdef CONFIG_OTG_USB_EN
		pinctrl_select_state(pinctrl, pinctrl_otgusben_high);
		#endif
	}
...
void mt_usb_otg_init(struct musb *musb) {}
void mt_usb_init_drvvbus(void){}
void mt_otg_usb_en(void){}/*otg_usb_en*/
void mt_usb_set_vbus(struct musb *musb, int is_on) {}
