...
static int __init item_map(void)
{
	...
	char *item;
	//void *va;
	printk("items: item_map  items_start = 0x%llx\n",items_start);
...
//		dram_sz += mem_desc->size;
	...
	items_size =  mem_desc->size;
	printk("items: of_get_flat_dt_prop  mem_desc->start = 0x%0x\n",(u32)mem_desc->start);
	printk("items: of_get_flat_dt_prop  mem_desc->size = 0x%0x\n",(u32)mem_desc->size);
