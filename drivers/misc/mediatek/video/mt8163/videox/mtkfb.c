...
static void mtkfb_update_screen_impl(void)
{
	...
	if (down_interruptible(&sem_overlay_buffer)) {
		pr_debug("[FB Driver] can't get semaphore in mtkfb_update_screen_impl()\n");
	} else {
		down_sem = true;
...
static int _convert_fb_layer_to_disp_input(struct fb_overlay_layer *src, disp_input_config *dst)
{
	...
	if (src->src_width > 0xffff || src->src_height > 0xffff ||
		src->src_width == 0 || src->src_height == 0 ||
		src->src_offset_x > 0xffff || src->src_offset_y > 0xffff ||
		src->tgt_offset_x > 0xffff || src->tgt_offset_x > 0xffff) {
		DISPERR("Invalid input parameters");
		return -1;
	}
	...
	"_convert_fb_layer_to_disp_input():id=%u, en=%u, next_idx=%u, vaddr=%p, paddr=%p, src fmt=%u, dst fmt=%u, pitch=%u, xoff=%u, yoff=%u, w=%u, h=%u\n",
	     dst->layer_id, dst->layer_enable, dst->next_buff_idx, dst->src_base_addr,
	...
	case MTKFB_POWEROFF:
		{
			MTKFB_FUNC();
			if (primary_display_is_sleepd()) {
				pr_debug("[FB Driver] is still in MTKFB_POWEROFF!!!\n");
				return r;
			}
	...
	case MTKFB_POWERON:
		{
			...
			if (primary_display_is_alive()) {
				pr_debug("[FB Driver] is still in MTKFB_POWERON!!!\n");
				return r;
			}
	...
	case MTKFB_SLT_AUTO_CAPTURE:
		{
			...
			if (copy_from_user(&capConfig, (void __user *)arg, sizeof(capConfig))) {
				...
				if (!src_pbuf) {
					...
					primary_display_capture_framebuffer_ovl((unsigned long)src_pbuf, format);
	...
	case MTKFB_META_RESTORE_SCREEN:
		{
			...
			if (var.width > 4096 || var.height > 4096 ||
				var.yoffset > 4096 || var.xoffset > 4096)
				return -EINVAL;
...
#define MTKFB_GET_POWERSTATE_32 MTK_IOR(21, unsigned int)
	...
	pr_debug("mtkfb_compat_ioctl, info=%p, cmd nr=0x%08x, cmd size=0x%08x\n", info,
		 (unsigned int)_IOC_NR(cmd), (unsigned int)_IOC_SIZE(cmd));
...
static int mtkfb_pan_display_proxy(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	pr_info("AppLaunch " "mtkfb_pan_display_proxy.\n");
#endif
	return mtkfb_pan_display_impl(var, info);
}
#if 0
...
char *mtkfb_find_lcm_driver(void)
{
	...
	if (1 == _parse_tag_videolfb()) {
		pr_debug("[mtkfb] mtkfb_lcm_name= %s\n",mtkfb_lcm_name);
...
static int mtkfb_probe(struct device *dev)
{
	...
	/* lk is ready , disable internal test */
#endif
...
#ifdef FPGA_DEBUG_PAN
	{
		...
		for (cnt = 0; cnt < fbsize; cnt++)
			*(fb_va++) = 0xFF00FF00;
#else
		memset(fb_va + fbsize * 4, 0x33, fbsize * 4);
...
static void mtkfb_late_resume(struct early_suspend *h)
{
	...
	if (down_interruptible(&sem_early_suspend)) {
		pr_debug("[FB Driver] can't get semaphore in mtkfb_late_resume()\n");
		mutex_unlock(&ScreenCaptureMutex);
		return;
	}

	pr_debug("[FB LR] 1\n");
	DISP_CHECK_RET(primary_display_suspend());
...
#if 0   /*no support ealry suspend in kernel 3.18*/
#ifdef CONFIG_HAS_EARLYSUSPEND
