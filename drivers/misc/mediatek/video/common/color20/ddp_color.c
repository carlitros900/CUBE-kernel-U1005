...
#include "ddp_color.h"
#include "cmdq_def.h"
#include <items.h> // mid Modify for replace MTK_LCM_PHYSICAL_ROTATION  with ro.sf.hwrotation
...
static void ddp_color_set_window(DISP_PQ_WIN_PARAM *win_param, void *__cmdq)
{
	...
	void *cmdq = __cmdq;
	// mid Modify for replace MTK_LCM_PHYSICAL_ROTATION  with ro.sf.hwrotation @{
	static int hwrotation = -1;
	if(hwrotation == -1){
		hwrotation = item_integer("ro.sf.hwrotation",0);
		if(0 != hwrotation && 90 != hwrotation && 180 != hwrotation && 270 != hwrotation){
			hwrotation = 0;
		}		
	}
    //mid @}
	...
	if (win_param->split_en) {
		g_split_en = 1;
if (180 == hwrotation) {  // #ifdef LCM_PHYSICAL_ROTATION_180
