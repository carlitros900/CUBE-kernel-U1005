...
const int camera_i2c_bus_num1 = 5;
const int camera_i2c_bus_num2 = 5;
...
PowerUp PowerOnList = {
	{
	 ...
	 {SENSOR_DRVNAME_GC8024SUB_MIPI_RAW,
	  {
	   {PDN, Vol_High, 1},
	   {RST, Vol_Low, 1},
	   {DOVDD, Vol_1800, 10},
	...
	if (On) {
		...
		for (pwListIdx = 0; pwListIdx < 16; pwListIdx++) {
			if (currSensorName && (PowerOnList.PowerSeq[pwListIdx].SensorName != NULL)
				...
				PK_DBG("sensorIdx:%d, pwListIdx=%d\n", SensorIdx, pwListIdx);
		...
		for (pwListIdx = 0; pwListIdx < 16; pwListIdx++) {
			if (currSensorName && (PowerOnList.PowerSeq[pwListIdx].SensorName != NULL)
			    && (0 == strcmp(PowerDownList.PowerSeq[pwListIdx].SensorName, currSensorName))) {
				PK_DBG("sensorIdx:%d, pwListIdx=%d\n", SensorIdx, pwListIdx);
				for (pwIdx = 0; pwIdx < 10; pwIdx++) {
					if (PowerDownList.PowerSeq[pwListIdx].PowerInfo[pwIdx].PowerType != VDD_None) {
						if (hwpowerdown(PowerDownList.PowerSeq[pwListIdx].PowerInfo[pwIdx], mode_name) == FALSE)
							goto _kdCISModulePowerOn_exit_;
						if (pwIdx > 0) {
							if (PowerDownList.PowerSeq[pwListIdx].PowerInfo[pwIdx - 1].Delay > 0)
