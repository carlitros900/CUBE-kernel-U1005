...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = BF2206MIPI_SENSOR_ID,	//0x2206	//record sensor id defined in Kd_imgsensor.h
...
/* SENSOR FLIP */
#define bf2206_HFLIP_EN    0
#define bf2206_VFLIP_EN    0
		...
		do {
			*sensor_id = return_sensor_id();
			if( *sensor_id == imgsensor_info.sensor_id ) {
				printk( "[bf2206] i2c_write_id=0x%x, sensor_id=0x%x\n", imgsensor.i2c_write_id, *sensor_id );
				printk( "<< get_imgsensor_id(): OK\n" );
				return ERROR_NONE;
			}
			printk( "[bf2206] Read sensor id fail, write_id=0x%x, sensor_id=0x%x\n", imgsensor.i2c_write_id, *sensor_id );
			retry--;
		} while( retry > 0 );
