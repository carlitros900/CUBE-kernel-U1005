...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC030AMIPI_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
#define ANALOG_GAIN_5 242 //3.78125
	...
	if((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2))
	{
		...
		LOG_INF("GC030A analogic gain 1x , GC030A add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_2<= iReg)&&(iReg < ANALOG_GAIN_3))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC030A analogic gain 1.42x , GC030A add pregain = %d\n",temp);		
	}
	else if((ANALOG_GAIN_3<= iReg)&&(iReg < ANALOG_GAIN_4))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC030A analogic gain 1.97x , GC030A add pregain = %d\n",temp);		
	}
	else if((ANALOG_GAIN_4<= iReg)&&(iReg < ANALOG_GAIN_5))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC030A analogic gain 2.78x , GC030A add pregain = %d\n",temp);		
	}
	else 
	{
		//analog gain
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC030A analogic gain 3.78x , GC030A add pregain = %d\n",temp);		
	}
	return gain;
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x1d, 0x48); // 20160323 GCFAE_huyingping    0x40 
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("GC030A get_imgsensor_id i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("GC030A get_imgsensor_id Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
