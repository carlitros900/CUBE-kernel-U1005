...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = SP250AMIPI_SENSOR_ID,
...
//#define DEBUG_SENSOR
#ifdef DEBUG_SENSOR
	#define SP250aMIPI_OP_CODE_INI		0x00		/* Initial value. */
	#define SP250aMIPI_OP_CODE_REG		0x01		/* Register */
	#define SP250aMIPI_OP_CODE_DLY		0x02		/* Delay */
	#define SP250aMIPI_OP_CODE_END		0x03		/* End of initial setting. */
	...
	} SP250aMIPI_initial_set_struct;

	SP250aMIPI_initial_set_struct SP250aMIPI_Init_Reg[1000];
 ...
 static u32 strtol(const char *nptr, u8 base)
{
 ...
 static u8 SP250aMIPI_Initialize_from_T_Flash(void)
{
	...
	fp = filp_open("/mnt/sdcard/sp250a_sd", O_RDONLY , 0); 
	if (IS_ERR(fp)) 
	{ 
	...
	while (curr_ptr < (data_buff + file_size))
	{
		...
		if (strcmp((const char *)func_ind, "REG") == 0)		/* REG */
		{
			...
			SP250aMIPI_Init_Reg[i].op_code = SP250aMIPI_OP_CODE_REG;
			
			SP250aMIPI_Init_Reg[i].init_reg = strtol((const char *)curr_ptr, 16);
			curr_ptr += 5;	/* Skip "00, 0x" */
		
			SP250aMIPI_Init_Reg[i].init_val = strtol((const char *)curr_ptr, 16);
			curr_ptr += 4;	/* Skip "00);" */
		...
		else 									/* DLY */
		{
			...
			curr_ptr += 4;	
			SP250aMIPI_Init_Reg[i].op_code = SP250aMIPI_OP_CODE_DLY;
			
			SP250aMIPI_Init_Reg[i].init_reg = 0xFF;
			SP250aMIPI_Init_Reg[i].init_val = strtol((const char *)curr_ptr,  10);	/* Get the delay ticks, the delay should less then 50 */
		}
		i++;
	...
	SP250aMIPI_Init_Reg[i].op_code = SP250aMIPI_OP_CODE_END;
	SP250aMIPI_Init_Reg[i].init_reg = 0xFF;
	SP250aMIPI_Init_Reg[i].init_val = 0xFF;
	i++;
	//for (j=0; j<i; j++)
		//printk(" %x  ==  %x\n",sp250aMIPI_Init_Reg[j].init_reg, sp250aMIPI_Init_Reg[j].init_val);

	/* Start apply the initial setting to sensor. */
	#if 1
	for (j=0; j<i; j++)
	{
		if (SP250aMIPI_Init_Reg[j].op_code == SP250aMIPI_OP_CODE_END)	/* End of the setting. */
		{
			break ;
		}
		else if (SP250aMIPI_Init_Reg[j].op_code == SP250aMIPI_OP_CODE_DLY)
		{
			msleep(SP250aMIPI_Init_Reg[j].init_val);		/* Delay */
		}
		else if (SP250aMIPI_Init_Reg[j].op_code == SP250aMIPI_OP_CODE_REG)
		{
		
			write_cmos_sensor(SP250aMIPI_Init_Reg[j].init_reg, SP250aMIPI_Init_Reg[j].init_val);
			printk("%x = %x\n",SP250aMIPI_Init_Reg[j].init_reg,SP250aMIPI_Init_Reg[j].init_val);
		}
		else
		{
   	 ...
   	 {   
   	    	 iReg = 0x10 * gain/BASEGAIN;        //change mtk gain base to aptina gain base              
						...
						if(iReg >= 0x80 && imgsensor.shutter >= 0xb8d)
						{
							...
							LOG_INF("SP2509MIPI_SetGain 1111111111--0x1f = 0x%x\n",read_cmos_sensor(0x1f));
							LOG_INF("SP2509MIPI_SetGain 1111111111--0x20 = 0x%x\n",read_cmos_sensor(0x20));
							LOG_INF("SP2509MIPI_SetGain 1111111111--0x21 = 0x%x\n",read_cmos_sensor(0x21));
							LOG_INF("SP2509MIPI_SetGain 1111111111--0x28 = 0x%x\n",read_cmos_sensor(0x28));
							LOG_INF("SP2509MIPI_SetGain 1111111111--0x29 = 0x%x\n",read_cmos_sensor(0x29));
						}
						else
						{
							write_cmos_sensor(0xfd, 0x02);
...
static void sensor_init(void)
{
	...
	if(fromsd == 1)//是否从SD读取//gepeiwei   120903
		...
		SP250aMIPI_Initialize_from_T_Flash();//从SD卡读取的主要函数
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
            printk("lingyq_sp250a gpw i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
...
static kal_uint32 open(void)
{
    ...
    if (imgsensor_info.sensor_id != sensor_id)
	    ...
	    {
		    fromsd = 1;
	          //sp250aMIPI_Initialize_from_T_Flash();
	          printk("gpww read ok!\n");
...
UINT32 SP250aMIPISensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    SP250aMIPISensorInit    */
