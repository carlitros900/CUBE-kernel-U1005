...
#include "gc5004raw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "GC5004_camera_sensor"
#define LOG_1 LOG_INF("GC5004,DVP\n")
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC5004_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h

    .checksum_value = 0x9d1c9dad,        //checksum value for Camera Auto Test

    .pre = {
        .pclk = 240000000,  //        .pclk = 24000000,                //record different mode's pclk
        .linelength = 4800,  //        .linelength = 4800,                //record different mode's linelength
    ...
    .normal_video = {
        .pclk = 240000000, //        .pclk = 24000000,
        .linelength = 4800, //        .linelength = 4800,
        .framelength = 1988,
        .startx = 0,
        .starty = 0,
    ...
    .cap_delay_frame = 6,  //3 tao      //enter capture delay frame num
...
{
	{ 1296, 972,	 0,    0, 1296, 972, 1296,  972, 0000, 0000, 1296,  972, 	 0,    0, 1296,  972}, // Preview 
	{ 2592, 1944,	 0,    0, 2592, 1944, 2592,  1944, 0000, 0000, 2592,  1944, 	 0,    0, 2592,  1944}, // capture 
	{ 1296, 972,	 0,    0, 1296, 972, 1296,  972, 0000, 0000, 1296,  972, 	 0,    0, 1296,  972}, // video 
	{ 1296, 972,	 0,    0, 1296, 972, 1296,  972, 0000, 0000, 1296,  972, 	 0,    0, 1296,  972}, //hight speed video 
	{ 1296, 972,	 0,    0, 1296, 972, 1296,  972, 0000, 0000, 1296,  972, 	 0,    0, 1296,  972}};// slim video 
...
//  modify by yfx
#if 0
	hb = imgsensor.dummy_pixel + GC5004_DEFAULT_DUMMY_PIXEL_NUMS;
	vb = imgsensor.dummy_line + GC5004_DEFAULT_DUMMY_LINE_NUMS;
...
#define ANALOG_GAIN_2 90  //1.37x
#define ANALOG_GAIN_3 128  // 1.89x
#define ANALOG_GAIN_4 178  // 2.59x
#define ANALOG_GAIN_5 247  // 3.70x
#define ANALOG_GAIN_6 332  // 5.06x
...
//			LOG_INF("GC5004 analogic gain 1x , GC5004 add pregain = %d\n",temp);
	...
	else if((ANALOG_GAIN_2<= iReg)&&(iReg < ANALOG_GAIN_3))
	{
			...
			write_cmos_sensor(0xb1, temp>>6);
			write_cmos_sensor(0xb2, (temp<<2)&0xfc);
//			LOG_INF("GC5004 analogic gain 1.45x , GC5004 add pregain = %d\n",temp);
	...
	else if((ANALOG_GAIN_3<= iReg)&&(iReg < ANALOG_GAIN_4))
	{
			...
			write_cmos_sensor(0xb1, temp>>6);
			write_cmos_sensor(0xb2, (temp<<2)&0xfc);
//			LOG_INF("GC5004 analogic gain 2.02x , GC5004 add pregain = %d\n",temp); 
	}
...
//			LOG_INF("GC5004 analogic gain 2.8x, temp = %d\n", temp);
	}
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0xf7, 0x9d); //Pll enable
	write_cmos_sensor(0xf8, 0x83); //Pll mode 2
	...
	////////////////   ANALOG & CISCTL   ////////////////
	/////////////////////////////////////////////////////
	write_cmos_sensor(0x00, 0x40); //[4]rowskip_skip_sh
	write_cmos_sensor(0x01, 0x10); // 20140324 txlow buffer
	write_cmos_sensor(0x03, 0x06);
	write_cmos_sensor(0x04, 0xd6);
	write_cmos_sensor(0x05, 0x01); //HB
	write_cmos_sensor(0x06, 0x0a);
	write_cmos_sensor(0x07, 0x00); //VB
	write_cmos_sensor(0x08, 0x1c);
	write_cmos_sensor(0x0a, 0x02); //row start
	write_cmos_sensor(0x0c, 0x00); //col start
	write_cmos_sensor(0x0d, 0x07); //Window setting
	write_cmos_sensor(0x0e, 0xa8);
	write_cmos_sensor(0x0f, 0x0a);
	write_cmos_sensor(0x10, 0x50);
	write_cmos_sensor(0x17, 0x15);//15 lc //[0]mirror [1]flip
	...
	write_cmos_sensor(0x1e, 0xb8);
	write_cmos_sensor(0x1f, 0x78);
	write_cmos_sensor(0x20, 0xc5); //[7:6]ref_r [3:1]comv_r
	write_cmos_sensor(0x21, 0x4f);
	write_cmos_sensor(0x22, 0xb2); // 82 20140722 
	write_cmos_sensor(0x23, 0x43); //[7:3]opa_r [1:0]sRef
	write_cmos_sensor(0x24, 0x2f); //PAD drive
	write_cmos_sensor(0x2b, 0x01);
	write_cmos_sensor(0x2c, 0x68); //[6:4]rsgh_r

	/////////////////////////////////////////////////////
	//////////////////////   ISP   //////////////////////
	/////////////////////////////////////////////////////
	write_cmos_sensor(0x86, 0x8e); //[7] data_delay_half
	write_cmos_sensor(0x89, 0x03); 
	write_cmos_sensor(0x8a, 0x83);
	write_cmos_sensor(0x8b, 0x61);
	write_cmos_sensor(0x8c, 0x10);
	write_cmos_sensor(0x8d, 0x01);
	write_cmos_sensor(0x90, 0x01);
	write_cmos_sensor(0x92, 0x00); //crop win y
	write_cmos_sensor(0x94, 0x0d); //crop win x
	write_cmos_sensor(0x95, 0x07); //crop win height
	write_cmos_sensor(0x96, 0x98);
	write_cmos_sensor(0x97, 0x0a); //crop win width
	write_cmos_sensor(0x98, 0x20);

	/////////////////////////////////////////////////////
	//////////////////////   BLK   //////////////////////
	/////////////////////////////////////////////////////
	write_cmos_sensor(0x40, 0x72); // 0x22 20140722
	...
	write_cmos_sensor(0x74, 0x20);
	write_cmos_sensor(0x75, 0x20);
	write_cmos_sensor(0x76, 0x20);
	write_cmos_sensor(0x77, 0x20);

	/////////////////////////////////////////////////////
	//////////////////////   GAIN   /////////////////////
	/////////////////////////////////////////////////////
	write_cmos_sensor(0xb0, 0x70);  // 50 20140722
	...
	write_cmos_sensor(0x80, 0x18);
	write_cmos_sensor(0x84, 0x23);
...
{
	//1296x972
	write_cmos_sensor(0x18, 0x42); //skip on
	write_cmos_sensor(0x80, 0x19); //scaler en
  	write_cmos_sensor(0xf7, 0x9d);
	write_cmos_sensor(0xf8, 0x84);
	write_cmos_sensor(0xfa, 0x00); //  20140722
	write_cmos_sensor(0x03, 0x06);
	write_cmos_sensor(0x04, 0xd6);
	write_cmos_sensor(0x05, 0x01);
	write_cmos_sensor(0x06, 0xfa);
	
	write_cmos_sensor(0x4e, 0x00);// blk 20140722
	write_cmos_sensor(0x4f, 0x06);
	write_cmos_sensor(0x0a, 0x03);
...
static void capture_setting(kal_uint16 currefps)
{
	...
	//2592x1944
	write_cmos_sensor(0x18, 0x02); //skip off
	write_cmos_sensor(0x80, 0x10); //scaler off
	write_cmos_sensor(0xf7, 0x9d);
	write_cmos_sensor(0xf8, 0x83);
	write_cmos_sensor(0xfa, 0x11); // pclk/2 20140722
	write_cmos_sensor(0x03, 0x06);
	write_cmos_sensor(0x04, 0xd6);
	write_cmos_sensor(0x05, 0x01);
	write_cmos_sensor(0x06, 0x0a); 
	...
	write_cmos_sensor(0x17, 0x15);//15 lc
	
	write_cmos_sensor(0x92, 0x00);//04
   	write_cmos_sensor(0x94, 0x0d);//0d
	write_cmos_sensor(0x95, 0x07);
	write_cmos_sensor(0x96, 0x98);
	write_cmos_sensor(0x97, 0x0a);
	write_cmos_sensor(0x98, 0x20);
...
static void normal_video_setting(kal_uint16 currefps)
{
	...
	//1296x972
	write_cmos_sensor(0x18, 0x42);//skip on
	write_cmos_sensor(0x80, 0x18);//scaler en

	write_cmos_sensor(0x05, 0x01); //HB
	write_cmos_sensor(0x06, 0xfa); 
	write_cmos_sensor(0x09, 0x00);	
	write_cmos_sensor(0x0a, 0x03); //row start
	write_cmos_sensor(0x0b, 0x00);
	write_cmos_sensor(0x0c, 0x00); //col start
	write_cmos_sensor(0x0d, 0x07); 
	write_cmos_sensor(0x0e, 0xa8); 
	write_cmos_sensor(0x0f, 0x0a); //Window setting
	write_cmos_sensor(0x10, 0x50); 
	write_cmos_sensor(0x4e, 0x00); //blk 20140722
	write_cmos_sensor(0x4f, 0x06); 
	...
	write_cmos_sensor(0x94, 0x07); //0d
	write_cmos_sensor(0x95, 0x03);
	write_cmos_sensor(0x96, 0xcc);
	write_cmos_sensor(0x97, 0x05);
	write_cmos_sensor(0x98, 0x10);
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("GC5004 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            printk("GC5004  Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
