...
#include "gc033amipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "GC033A_camera_sensor"
//#define LOG_1 LOG_INF("GC033a,MIPI 1LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC033AMIPI_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
{	
	...
	{ 
		...
		LOG_INF("GC033AMIPI analogic gain 1x, GC033AMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_2<= iReg)&&(iReg < ANALOG_GAIN_3))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC033AMIPI analogic gain 1.42x , GC033AMIPI add pregain = %d\n",temp);
	}
	...
	{ 
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC033AMIPI analogic gain 1.97x , GC033AMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_4<= iReg)&&(iReg < ANALOG_GAIN_5))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC033AMIPI analogic gain 2.78x , GC033AMIPI add pregain = %d\n",temp);
	}
	else 
	{
		//analog gain
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC033AMIPI analogic gain 3.78x , GC033AMIPI add pregain = %d\n",temp);
	}
	return gain;
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("GC033A get_imgsensor_id i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("GC033A get_imgsensor_id Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) 
    {
	    ...
	    if (sensor_id == imgsensor_info.sensor_id) 
	    {
	        LOG_INF("GC033a open i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
	   	 LOG_INF("GC033a open Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
...
UINT32 GC033AMIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    GC033AMIPI_RAW_SensorInit    */
