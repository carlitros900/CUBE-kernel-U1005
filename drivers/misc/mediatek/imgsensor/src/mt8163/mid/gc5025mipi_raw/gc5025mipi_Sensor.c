...
//static kal_uint32 Basic_Dummy_Line = GC5025_DEFAULT_DUMMY_LINE_NUMS;
...
static imgsensor_info_struct imgsensor_info = {
       ...
       .hs_video = {
        ...
        .max_frame_length = 0x1388,//max framelength by sensor register's limitation
...
{	
    kal_uint8 flag_doublereset,flag_GC5025A,flag_packversion;
	...
	flag_packversion = gc5025_read_otp(0x7f);
	
	if((flag_GC5025A&0x01 ) == 0x01) {
		LOG_INF("GC5025A sensor! \n");
		DR_State = KAL_FALSE;
    } else { 
    	if((flag_packversion & 0x07) == 0x02) {
			if((flag_doublereset & 0x03)==0x01)
				LOG_INF("COB packaged with double reset off\n");
			LOG_INF("COB packaged\n");	
			DR_State = KAL_TRUE;
		} else {
			if((flag_doublereset & 0x03)==0x01) {
				DR_State = KAL_FALSE;
	...
	//DR_State = KAL_FALSE; //for testing;
...
static void set_dummy(void)
{
	...
	vb = imgsensor.dummy_line * 2 + 0x08; /*Basic_Dummy_Line*/
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0xd3, 0xc4);//cc 20170814
	write_cmos_sensor(0xd9, 0xaa);
