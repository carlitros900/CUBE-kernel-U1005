...
#endif
#define PFX "sp5507_camera_sensor"
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			printk("tong1 , write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {				
				printk("tong1 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			printk("tong1 Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
