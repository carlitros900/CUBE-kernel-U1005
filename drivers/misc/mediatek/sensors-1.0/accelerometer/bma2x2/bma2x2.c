...
static const struct dev_pm_ops bma2x2_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(bma2x2_suspend, bma2x2_resume)
};
#endif
#endif
...
static int bma2x2_acc_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
    return bma2x2_set_delay((u64)samplingPeriodNs);
}

static int bma2x2_acc_flush(void)
{
	 ...
	 const struct i2c_device_id *id)
{
	...
	ctl.batch = bma2x2_acc_batch;
	ctl.flush = bma2x2_acc_flush;
