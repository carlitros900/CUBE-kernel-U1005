...
/**
 * 根据博世Sensor的方向来计算明皓的方向, 计算公式如下
 * bma值为0的情况下，mir3da为3
 * bma值为1/2/3的情况下，mir3da = bma - 1
 * bma值为4/5/6的情况下，mir3da = bma + 1
 * bma值为7 的情况下，mir3da为4
 * @param  mir3da [description]
 * @return        [description]
 */
int direction_calculate(int bma)
{
    int mir = 0;
    switch(bma){
        case 0:
...
static int mir3da_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    bma = item_integer("sensor.accel.mir3da.orientation", 0);
    bma = bma < 0 ? 0 : bma;
    obj->hw.direction = direction_calculate(bma);  // 
    printk("mc3xx: direction bma=%d, mc3xx=%d\n", bma, obj->hw.direction);
...
static const struct dev_pm_ops mir3da_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(mir3da_suspend, mir3da_resume)
};
#endif
*/
