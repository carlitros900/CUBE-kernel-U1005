...
static int _is_hdmi_decouple_mode(EXT_DISP_PATH_MODE mode)
{
...
static int _convert_disp_input_to_rdma(RDMA_CONFIG_STRUCT *dst, ext_disp_input_config *src)
{
	if (src && dst) {
		...
		dst->buf_offset = 0;
		if (1 == g_hdmi_is_interlace && !_is_hdmi_decouple_mode(pgc->mode)) {
			dst->height /= 2;
			dst->pitch *= 2;
		}
		return 0;
	}
	EXT_DISP_ERR("src(0x%p) or dst(0x%p) is null\n", src, dst);
	return -1;
}
...
static int _trigger_display_interface(int blocking, void *callback, unsigned int userdata)
{
	...
	if (_should_trigger_path()) {
		...
		udelay(1000);
		if ((!ovl_is_idle(DISP_MODULE_OVL1)) && (ovl_get_status() == DDP_OVL1_STATUS_SUB))
...
static int ext_disp_trigger_for_interlace(void)
{
	...
	if (!g_hdmi_is_interlace || _is_hdmi_decouple_mode(pgc->mode))
		return 0;
	...
	if (_should_config_ovl_input()) {
		for (i = 0; i < HDMI_OVERLAY_CNT; i++) {
			...
			if (data_config->ovl_config[i].layer_en && ddp_dpi_is_top_filed(DISP_MODULE_DPI1)) {
				data_config->ovl_config[i].src_x = data_config->ovl_config[i].src_ori_x
					+ data_config->ovl_config[i].src_pitch/bpp;
			} else {
				data_config->ovl_config[i].src_x = data_config->ovl_config[i].src_ori_x;
			}
		}
		...
		if (ddp_dpi_is_top_filed(DISP_MODULE_DPI1))
				data_config->rdma_config.buf_offset = ext_disp_get_width() * bpp;
		else
			data_config->rdma_config.buf_offset = 0;
...
static void ext_disp_frame_update_irq_callback(DISP_MODULE_ENUM module, unsigned int param)
{
	if (module == DISP_MODULE_RDMA1) {
		...
		if ((param & 0x2) && g_hdmi_is_interlace) {
			/* rdma0 frame start */
...
#ifdef MTK_EXT_DISP_RELEASE_FENCE_NEW
...
static int deinit_cmdq_slots(cmdqBackupSlotHandle *pSlot)
{
	cmdqBackupFreeSlot(*pSlot);
...
int ext_disp_init(char *lcm_name, unsigned int session)
{
	...
	dpmgr_init();
       item_string(lcmname,"LCM",0);

	extd_mutex_init(&(pgc->lock));
	_ext_disp_path_lock();

#ifdef MTK_EXT_DISP_RELEASE_FENCE_NEW
	init_cmdq_slots(&(extd_cur_config_fence), DISP_SESSION_TIMELINE_COUNT, 0);
	init_cmdq_slots(&(extd_subtractor_when_free), DISP_SESSION_TIMELINE_COUNT, 0);
#endif
...
/* EXT_DISP_LOG("ext_disp display is RDMA to dpi MODE\n"); */
	...
	if (data_config) {
		...
		memcpy(&(data_config->dispif_config), &(extd_dpi_params.dispif_config),
		       sizeof(LCM_PARAMS));
	...
	if (ext_disp_use_cmdq == CMDQ_ENABLE) {
		_cmdq_reset_config_handle();
		/* _cmdq_insert_wait_frame_done_token();*/
		/* //Fixed me: why insert EOF event before the first frame start */
	}
...
#ifdef MTK_EXT_DISP_RELEASE_FENCE_NEW
	deinit_cmdq_slots(&(extd_cur_config_fence));
	deinit_cmdq_slots(&(extd_subtractor_when_free));
#endif
...
int ext_disp_wait_for_vsync(void *config, unsigned int session)
{
	...
	if (pgc->state == EXTD_DEINIT) {
		EXT_DISP_LOG("vsync for ext display path not ready yet(0)\n");
		_ext_disp_path_unlock();
		msleep(20);
		return 0;
	}
	...
	if (pgc->dpmgr_handle == NULL) {
		EXT_DISP_LOG("vsync for ext display path not ready yet(1)\n");
		_ext_disp_path_unlock();
		return 0;
	}

	_ext_disp_vsync_lock(session);
	ret = dpmgr_wait_event_timeout(pgc->dpmgr_handle, DISP_PATH_EVENT_IF_VSYNC, 6);
...
int ext_disp_suspend(void)
{
	...
	if (!(DISP_REG_GET(DISP_REG_CONFIG_MMSYS_HW_DCM_DIS0) & 0x00000080)) {
...
int ext_disp_trigger(int blocking, void *callback, unsigned int userdata)
{
	...
	if ((is_hdmi_active() == false) || pgc->state == EXTD_DEINIT || pgc->state == EXTD_SUSPEND
	...
	if (g_hdmi_is_interlace && !_should_start_path()) {
		EXT_DISP_DBG("ext_disp_trigger return.\n");
		return 0;
	}
	...
	pgc->state = EXTD_RESUME;
	_ext_disp_path_unlock();
	EXT_DISP_DBG("ext_disp_trigger done\n");
...
int ext_disp_config_input(ext_disp_input_config *input)
{
	...
	if ((is_hdmi_active() == false) || ext_disp_is_sleepd()) {
		EXT_DISP_LOG("ext disp is already slept\n");
		return 0;
	}
	...
	if (input->layer_en) {
		if (input->vaddr) {
			/* _debug_pattern(0x00000000, input->vaddr, input->dst_w,*/
			/* input->dst_h, input->src_pitch, 0x00000000, input->layer, input->buff_idx);*/
		} else {
			/* _debug_pattern(input->addr,0x00000000,  input->dst_w, */
			/*input->dst_h, input->src_pitch, 0x00000000, input->layer, input->buff_idx);*/
		}
	}
...
int ext_disp_config_input_multiple(ext_disp_input_config *input, int idx)
{
	...
	if ((is_hdmi_active() == false) || (pgc->state != EXTD_INIT && pgc->state != EXTD_RESUME)) {
		EXT_DISP_LOG
		    ("[Donglei]config ext disp is already slept, hdmi_active:%d, state:%d\n",
		...
		for (i = 0; i < HDMI_OVERLAY_CNT; i++) {
			...
			if (init_roi == 1) {
				...
				memcpy(&(data_config->dispif_config),
				       &(extd_dpi_params.dispif_config), sizeof(LCM_PARAMS));
			...
			data_config->ovl_dirty = 1;
		#ifdef MTK_EXT_DISP_RELEASE_FENCE_NEW
			cmdqBackupReadSlot(extd_cur_config_fence, input[i].layer, &last_fence);
			cur_fence = input[i].buff_idx;

			if (cur_fence != -1 && cur_fence > last_fence)
				cmdqRecBackupUpdateSlot(pgc->cmdq_handle_config, extd_cur_config_fence, input[i].layer,
							cur_fence);
			/* for dim_layer/disable_layer/no_fence_layer, just release all fences configured */
			/* for other layers, release current_fence-1 */
			if (input[i].buff_source == DISP_BUFFER_ALPHA
			    || input[i].layer_en == 0 || cur_fence == -1)
				cmdqRecBackupUpdateSlot(pgc->cmdq_handle_config, extd_subtractor_when_free,
					input[i].layer, 0);
			else
				cmdqRecBackupUpdateSlot(pgc->cmdq_handle_config, extd_subtractor_when_free,
					input[i].layer, 1);
	...
	if (data_config->ovl_dirty) {
		EXT_DISP_DBG("config_input_multiple idx:%d -w:%d, h:%d, pitch:%d\n", idx,
			     data_config->ovl_config[0].src_w, data_config->ovl_config[0].src_h,
			     data_config->ovl_config[0].src_pitch);
	} else {
		EXT_DISP_DBG("config_input_multiple idx:%d -w:%d, h:%d, pitch:%d, mva:0x%lx\n", idx,
...
int extd_disp_hdmi_is_decouple(void)
{
	return _is_hdmi_decouple_mode(pgc->mode);
}

int extd_disp_get_mutex_id(void)
{
