...
static irqreturn_t thr_h_int_handler(int irq, void *dev_id)
{
	...
	/* Read PMIC VMCH OC Status */
	ret = pmic_read_interface(0x520, &val, 0x1, 6);
	if (val == 1) {
		/* VMCH over current occur */
		pr_notice("%s: VMCH OC with status: 0x%x, turn off!\n",
			__func__, upmu_get_reg_value(0x520));
...
void PMIC_INIT_SETTING_V1(void)
{
	...
	if (chip_version >= PMIC6323_E2_CID_CODE) {
		...
		#ifdef HIGH_BATTERY_VOLTAGE_SUPPORT
		upmu_set_rg_vbat_ov_vth(0x3);	/*VBAT_OV_VTH, 4.45V,*/
		#if defined(CONFIG_MTK_SWCHR_SUPPORT)
			upmu_set_rg_vbat_ov_en(0);
		#endif
		#else
