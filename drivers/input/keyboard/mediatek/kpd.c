...
//		if(item_integer("sensor.hall.level",0) != 1)
...
struct of_device_id hall_of_match[] = {
	{ .compatible = "mediatek,mt8176-hall", },
	{},
};
...
//	mt_eint_unmask(CUST_EINT_MHALL_NUM);
	...
	if (node) {
		...
		of_property_read_u32_array(node, "debounce", deb, ARRAY_SIZE(deb));
		hall_gpiopin = deb[0];
		hall_debounce = deb[1];
		pr_err("hall_gpiopin = %d,hall_debounce = %d\n",hall_gpiopin,hall_debounce );
		gpio_set_debounce(hall_gpiopin, hall_debounce);
		ret = request_irq(hall_irq,(irq_handler_t)kpd_hall_eint_handler_event, IRQF_TRIGGER_NONE, "HALL-eint", NULL);
		if (ret > 0) {
			ret = -1;
			pr_err("hall request_irq IRQ LINE NOT AVAILABLE!.");
		}
	}
	//enable_irq(hall_irq);
...
#endif
// tongjun@roco Add for hall @{
//	if(item_integer("sensor.hall.exist",0) != 0)
//	{
		pr_err("sensor.hall.exist == 1\n");
		mt_hall_eint_register();
//	}
//	else
//		pr_err(KPD_SAY "sensor.hall.exist == 0\n");		
// tongjun@roco Add for hall @}
