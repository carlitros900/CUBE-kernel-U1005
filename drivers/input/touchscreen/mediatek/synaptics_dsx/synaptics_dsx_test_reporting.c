...
static struct synaptics_rmi4_create_work create_attr_test;
...
static void remove_sysfs(void)
{
	...
	cancel_delayed_work_sync(&create_attr_test.work);
	flush_workqueue(create_attr_test.workqueue);
	destroy_workqueue(create_attr_test.workqueue);
...
static void synaptics_rmi4_f54_set_sysfs(struct work_struct *work)
{
	int retval;
	int reg_num;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	...
	f54->attr_dir = kobject_create_and_add("f54",
			&tpd->dev->dev.kobj /*rmi4_data->input_dev->dev.kobj*/);
	if (!f54->attr_dir) {
		dev_err(&rmi4_data->i2c_client->dev,
...
pdt_done:
	...
	create_attr_test.workqueue = create_singlethread_workqueue("f54_sysfs_workqueue");
	INIT_DELAYED_WORK(&create_attr_test.work, synaptics_rmi4_f54_set_sysfs);
	create_attr_test.rmi4_data = f54->rmi4_data;
	create_attr_test.queue_work = true;
	queue_delayed_work(create_attr_test.workqueue,
			&create_attr_test.work,
