...
static struct synaptics_rmi4_create_work create_attr;
static struct synaptics_rmi4_data *g_rmi4_ptr;
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->sensor_sleep && rmi4_data->enable_wakeup_gesture) {
		...
		if (detected_gestures) {
			input_report_key(tpd->dev /*rmi4_data->input_dev*/, KEY_POWER, 1);
			input_sync(tpd->dev /*rmi4_data->input_dev*/);
			input_report_key(tpd->dev /*rmi4_data->input_dev*/, KEY_POWER, 0);
			input_sync(tpd->dev /*rmi4_data->input_dev*/);
	...
	for (finger = 0; finger < fingers_supported; finger++) {
		...
		input_mt_slot(tpd->dev /*rmi4_data->input_dev*/, finger);
		input_mt_report_slot_state(tpd->dev /*rmi4_data->input_dev*/,
		...
		if (finger_status) {
			...
			input_report_key(tpd->dev /*rmi4_data->input_dev*/,
					BTN_TOUCH, 1);
			input_report_key(tpd->dev /*rmi4_data->input_dev*/,
					BTN_TOOL_FINGER, 1);
			input_report_abs(tpd->dev /*rmi4_data->input_dev*/,
					ABS_MT_POSITION_X, x);
			input_report_abs(tpd->dev /*rmi4_data->input_dev*/,
					ABS_MT_POSITION_Y, y);
#ifdef REPORT_2D_W
			...
			input_mt_sync(tpd->dev /*rmi4_data->input_dev*/);
#endif
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		if (finger_status) {
            ...
            //printk("synaptics_rmi4_f12_abs_report Number of fingers to process, x= %d, y=%d ,sensor_max_x=%d , sensor_max_y=%d\n",x , y, rmi4_data->sensor_max_x, rmi4_data->sensor_max_y);
		...
		unsigned int intr_count)
{
	...
	struct synaptics_rmi4_f11_query_7_8 *query_7_8 = NULL;
	struct synaptics_rmi4_f11_query_9 *query_9 = NULL;
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
	...
	input_set_abs_params(tpd->dev /*rmi4_data->input_dev*/,
			ABS_MT_POSITION_X, 0,
	...
	input_mt_init_slots(tpd->dev /*rmi4_data->input_dev*/,
	...
	if (f1a) {
		for (ii = 0; ii < f1a->valid_button_count; ii++) {
			set_bit(f1a->button_map[ii],
					tpd->dev->keybit /*rmi4_data->input_dev->keybit*/);
			input_set_capability(tpd->dev /*rmi4_data->input_dev*/,
					EV_KEY, f1a->button_map[ii]);
		}
	}

	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture) {
		set_bit(KEY_POWER, tpd->dev->keybit /*rmi4_data->input_dev->keybit*/);
		input_set_capability(tpd->dev /*rmi4_data->input_dev*/, EV_KEY, KEY_POWER);
	}
	return;
}
...
static int synaptics_rmi4_free_fingers(struct synaptics_rmi4_data *rmi4_data)
{
	...
	for (ii = 0; ii < rmi4_data->num_of_fingers; ii++) {
		input_mt_slot(tpd->dev /*rmi4_data->input_dev*/, ii);
		input_mt_report_slot_state(tpd->dev /*rmi4_data->input_dev*/,
				MT_TOOL_FINGER, 0);
	}
#endif
...
/**
* synaptics_rmi4_create_sysfs_work()
*
* This function is a work thread that creates attribute files
* for the object of mtk_tpd inpudt device
*/
static void synaptics_rmi4_create_file_work(struct work_struct *work)
{
	unsigned char attr_count;
	int retval;
	struct synaptics_rmi4_data *rmi4_data = create_attr.rmi4_data;

	dev_info(&rmi4_data->i2c_client->dev, "%s: entry\n", __func__);

	if (!tpd_register_flag) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: mtk_tpd input device is not registered\n",
				__func__);
		return;
	}
...
#endif
{
	...
	// create a dely work to add the attributes onto the mtk_tpd
	create_attr.workqueue = create_singlethread_workqueue("dsx_sysfs_workqueue");
	INIT_DELAYED_WORK(&create_attr.work, synaptics_rmi4_create_file_work);
	create_attr.rmi4_data = rmi4_data;
	create_attr.queue_work = true;
	queue_delayed_work(create_attr.workqueue,
			&create_attr.work,
			msecs_to_jiffies(CREATE_FILE_WORK_DELAY_MS));
...
err_sysfs:
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		sysfs_remove_file(&tpd->dev->dev.kobj /*&rmi4_data->input_dev->dev.kobj*/,
				&attrs[attr_count].attr);
	}

	cancel_delayed_work_sync(&create_attr.work);
	flush_workqueue(create_attr.workqueue);
	destroy_workqueue(create_attr.workqueue);
...
static struct i2c_driver tpd_i2c_driver = {
	...
	.driver = {
		   .name = "synaptics_dsx",
		   .of_match_table = synaptics_dt_match,
		   },
