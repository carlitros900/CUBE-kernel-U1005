...
static struct synaptics_rmi4_create_work create_attr_fwu;
...
static int fwu_get_device_config_id(void)
{
	...
	dev_info(&fwu->rmi4_data->i2c_client->dev,
			"%s: xuchen fwu->rmi4_data->config_id = %d\n",
			__func__, fwu->rmi4_data->config_id);
	*/
	return 0;
}
...
static enum flash_area fwu_go_nogo(void)
{
	...
	if (fwu->bl_version == BL_V7 || fwu->bl_version == BL_V8)
		...
		config_id_size = V5V6_CONFIG_ID_SIZE;
             /*
				dev_info(&fwu->rmi4_data->i2c_client->dev,
			"%s: xuchen fwu->img.ui_config.dat = %d\n",
			__func__, be_to_uint(fwu->img.ui_config.data));
			*/
	
	for (ii = 0; ii < config_id_size; ii++) {
		    /*
			dev_info(&fwu->rmi4_data->i2c_client->dev,
			"%s: fwu->img.ui_config.data[ii] = %d\n",
			__func__, fwu->img.ui_config.data[ii]);
				dev_info(&fwu->rmi4_data->i2c_client->dev,
			"%s: fwu->config_id[ii] = %d\n",
			__func__, fwu->config_id[ii]);
			*/
...
static void synaptics_rmi4_fwu_create_file_work(struct work_struct *work)
{
	unsigned char attr_count;
	int retval;
	struct synaptics_rmi4_data *rmi4_data = create_attr_fwu.rmi4_data;
	...
	retval = sysfs_create_bin_file(&tpd->dev->dev.kobj,
			&dev_attr_data);
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Failed to create sysfs bin file\n",
				__func__);
		goto exit_remove_qorkqueue;
	}
...
exit_remove_attrs:
	for (attr_count--; attr_count >= 0; attr_count--)
		sysfs_remove_file(&tpd->dev->dev.kobj, &attrs[attr_count].attr);

	sysfs_remove_bin_file(&tpd->dev->dev.kobj, &dev_attr_data);
exit_remove_qorkqueue:
	cancel_delayed_work_sync(&create_attr_fwu.work);
	flush_workqueue(create_attr_fwu.workqueue);
	destroy_workqueue(create_attr_fwu.workqueue);
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	create_attr_fwu.workqueue = create_singlethread_workqueue("fwu_sysfs_workqueue");
	INIT_DELAYED_WORK(&create_attr_fwu.work, synaptics_rmi4_fwu_create_file_work);
	create_attr_fwu.rmi4_data = rmi4_data;
	create_attr_fwu.queue_work = true;
	queue_delayed_work(create_attr_fwu.workqueue,
			&create_attr_fwu.work,
...
static void synaptics_rmi4_fwu_remove(struct synaptics_rmi4_data *rmi4_data)
{
	...
	sysfs_remove_bin_file(&tpd->dev->dev.kobj /*rmi4_data->input_dev->dev.kobj*/, &dev_attr_data);
