...
int gsl_gesture_flag = 0;   //@tony add
...
#ifdef TP_COMPATIBLE
	ret = tpd_hwPowerControl(3000000, TP_POWER, 1);
#else	
...
if(item_integer("sensor.tp.proximity",0)==1)
{
	...
	printk("In %s , gsl_seperate_type = %d \n", __func__, gsl_seperate_type);
#ifdef GSL_GESTURE
	if(gsl_gesture_flag == 1){
		u8 buf[4];
//		unsigned int temp;
...
static int tp_alsps_init(void)
{
	...
	ret = kobject_init_and_add(&kobj,&ktype,NULL,"mid");
	if(!ret)
		printk(" tp kobject_init_and_add  \"/sys/mid\" successful! \n");
	else
		printk(" tp kobject_init_and_add  \"/sys/mid\" failed! \n");
	return 0;
...
static int __init tpd_driver_init(void) {
	...
	printk("[TouchScreen] Start \n\t Sileadinc gslX680 touch panel driver init \n");
	
/*	
	if(item_exist("touchpanel.reset.pin"))
		{rst = item_integer("touchpanel.reset.pin",0);gpio_ctp_rst_pin = (rst | 0x80000000);}
	else
		gpio_ctp_rst_pin = GPIO_CTP_RST_PIN;


	if(item_exist("touchpanel.eint.pin"))
		{eint = item_integer("tp.eint.pin",0);gpio_ctp_eint_pin = (eint | 0x80000000);}
	else
		gpio_ctp_eint_pin = GPIO_CTP_EINT_PIN;

	
	if(item_exist("touchpanel.eint.num"))
		eint_num = item_integer("tp.eint.num",0);
	else
		eint_num = CUST_EINT_TOUCH_PANEL_NUM;
*/
