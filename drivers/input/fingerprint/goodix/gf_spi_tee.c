...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,goodix_chips");
	if (IS_ERR(node)) {
	...
	if (IS_ERR(gf_dev->pinctrl_gpios)) {
		...
		return  PTR_ERR(gf_dev->pinctrl_gpios);
	}
	...
	gf_dev->pins_miso_spi = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fingerprint_miso_spi");
	if (IS_ERR(gf_dev->pins_miso_spi)) {
		ret = PTR_ERR(gf_dev->pins_miso_spi);
		gf_debug(ERR_LOG, "%s can't find fingerprint pinctrl miso_spi\n", __func__);
		return ret;
	}
	gf_dev->pins_miso_pullhigh = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fingerprint_miso_pullhigh");
	...
	gf_dev->pins_miso_pulllow = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fingerprint_miso_pulllow");
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	unsigned char rx_test[10] = {0};//for spi test
	...
	*/
	...
	case GF_IOC_GET_FW_INFO:
		...
		gf_spi_read_bytes_ree(gf_dev, 0x0000, 4, rx_test);//for spi test
...
static int gf_probe(struct spi_device *spi)
{
	...
	FUNC_ENTRY();
	printk("Setting gf device configuration==========\n");
...
static int __init gf_init(void)
{
	...
	gf_debug(ERR_LOG, "%s, end to register SPI driver.\n", __func__);
	if (status < 0) {
