...
void bluewhale_set_ta_current_pattern_extern(unsigned int increase)
{
	...
	bluewhale_set_vbus_current(charger_info, 700);
	...
	if(increase)
	{
		...
		//mdelay(85);
		usleep_range(95*1000,100*1000);
		
		//pr_err("mtk_ta_increase() start\n");

		bluewhale_set_vbus_current(charger_info, 700); /* 700mA */
		...
		//mdelay(281);
		usleep_range(290*1000,300*1000);
		...
		//mdelay(440);
		usleep_range(450*1000,490*1000);

		bluewhale_set_vbus_current(charger_info, 100); /* 100mA */
		//pr_err("mtk_ta_increase() off 6");
		//mdelay(50);
		usleep_range(45*1000,50*1000);

		pr_notice("mtk_ta_increase() end \n");

		bluewhale_set_vbus_current(charger_info, 700); /* 700mA */
		//mdelay(200);
		usleep_range(190*1000,200*1000);
	}
	else
	{
		bluewhale_set_vbus_current(charger_info, 100); /* 100mA */
...
unsigned int bluewhale_set_ta20_reset_extern(void)
{
	...
	ilimit_org = bluewhale_get_vbus_current(charger_info);
	cc_org = bluewhale_get_charger_current(charger_info);
	...
	bluewhale_set_vbus_current(charger_info, ilimit_org);
	bluewhale_set_charger_current(charger_info, cc_org);
...
#else
	vbus_voltage = 5000;
	bluewhale_dbg("no ADC to measure vbus voltage,but we need"
		" test the funciton,so we set the vbus_voltage 5000");
#endif
...
#define ADJI_JUDG_IENOUGH_IBATTH 	(-100)
...
#if 0//O2_CONFIG_TRUSTED_VBUS_SUPPORT
...
#define ADJI_AVG_INFO_FIRST_DELAY	(70)
#endif
...
if(o2_ytag_support)
{
	...
	for(i=0; i < (ADJI_ILELS_NUM); i++) {
		...
		if( adji_ilimit_buffer != 0 && adji_ilimit_buffer < adji_ilel_array[i]) {
            ...
            ilimit_dbg("[%s]fall back to lower ilimit", __FUNCTION__);
			if(i>0) i-- ;
			goto adji_end ;
		}	
...
#endif
					{
                        ilimit_dbg("[%s] ADJI_RET_IENOUGH return ",__FUNCTION__);
...
static void o2_stop_charge(struct bluewhale_charger_info *charger, u8 cable_out)
{
	...
	if (!cable_out)
	{
		...
		ret = bluewhale_set_chg_volt(charger, 4200);
...
//			ret = gpio_request(charger->otg_en_gpio, "otg_en_gpio");        
			...
			else
			{
				...
				gpiod_direction_output(otg_en_gpiod, 1); //configure gpio mode: output
#if 0
				msleep(500);
				bluewhale_dbg("%s: otg_en_gpio:%d", __func__,
						gpio_get_value(charger->otg_en_gpio));
