...
extern unsigned int Inflag; 
extern unsigned int pollinglater;
...
// printk("xuchen mmc_power_up=================\n");
//xuchen add begin 20180826
if (cd_gpio&&pollinglater)
{
// printk("xuchen mmc_power_up cd_gpio = %d\n",cd_gpio);

    if(!__gpio_get_value(cd_gpio)&&Inflag==0){
            upmu_set_rg_vmch_en(1);
               Inflag = 1;     
            pr_err("xuchen sd mmc_sd_detect in \n");
    }
else if (__gpio_get_value(cd_gpio)&&Inflag==1){        
            upmu_set_rg_vmch_en(1);
               Inflag = 0;             
            pr_err("xuchen sd mmc_sd_detect out \n");
       }
}
//xuchen add end 20180826		   
