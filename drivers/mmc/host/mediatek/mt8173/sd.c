...
#define DEBUG_LOG_IN_ARRAY
#ifdef DEBUG_LOG_IN_ARRAY
...
static struct log_item log_items[MAX_LOG_NUM];
static bool log_override;
static bool log_dumping;
static u32 log_pos_idx;
...
void dump_debug_log_array(void)
{
	int i;

	log_dumping = true;
	pr_err("sdio dump log: over:%d, pos:%d\n", log_override, log_pos_idx);
	if (log_override)
		for (i = log_pos_idx + 1; i < MAX_LOG_NUM; i++)
			pr_err("sdio dbg[%d]-[%ld]: [%s] [0x%8x] [0x%8x] [0x%8x]\n",
			       i, log_items[i].time, log_items[i].msg,
			       log_items[i].val1, log_items[i].val2,
			       log_items[i].val3);

	for (i = 0; i <= log_pos_idx; i++)
		pr_err("sdio dbg[%d]-[%ld]: [%s] [0x%8x] [0x%8x] [0x%8x]\n",
		       i, log_items[i].time, log_items[i].msg,
		       log_items[i].val1, log_items[i].val2,
		       log_items[i].val3);
...
EXPORT_SYMBOL(dump_debug_log_array);

void add_debug_log(const char *msg, u32 val1, u32 val2, u32 val3)
{
	int len = 0;
	...
	if (msg) {
		len = strlen(msg);
		len = (len >= (MAX_MSG_SIZE - 1)) ? MAX_MSG_SIZE - 1 : len;
		memcpy(log_items[log_pos_idx].msg, msg, len);
		log_items[log_pos_idx].msg[MAX_MSG_SIZE - 1] = '\0';
	} else {
		log_items[log_pos_idx].msg[0] = '\0';
	}
	log_items[log_pos_idx].time = get_time_us();
	log_items[log_pos_idx].val1 = val1;
	log_items[log_pos_idx].val2 = val2;
	log_items[log_pos_idx].val3 = val3;

	/* pr_err("sdio_log[%d]-[%ld]: [%s] [0x%x] [0x%x] [0x%x]\n",
	 *         log_pos_idx, log_items[log_pos_idx].time,
	 *         msg, val1, val2, val3);
	 */

	if (log_pos_idx == MAX_LOG_NUM - 1)
		log_override = true;
	log_pos_idx = (log_pos_idx + 1) % MAX_LOG_NUM;
}
EXPORT_SYMBOL(add_debug_log);
#endif
...
void msdc_dump_register(struct msdc_host *host)
{
	...
	pr_err("SDIO TOP_DAT_TUNE_CTRL1=0x%.8x\n",
			sdr_read32(host->top + MSDC_TOP_DAT_TUNE_CTRL1));
	pr_err("SDIO TOP_DAT_TUNE_CTRL2=0x%.8x\n",
			sdr_read32(host->top + MSDC_TOP_DAT_TUNE_CTRL2));
	pr_err("SDIO TOP_DAT_TUNE_CTRL3=0x%.8x\n",
			sdr_read32(host->top + MSDC_TOP_DAT_TUNE_CTRL3));
...
void msdc_prepare_clk(struct msdc_host *host)
{
	if (!host->clk_on) {
		if (host->bus_clk_ctrl)
			clk_prepare(host->bus_clk_ctrl);
		if (host->src_clk_ctrl)
			clk_prepare(host->src_clk_ctrl);
		host->clk_on = true;
		clk_cnt = 0;
	}
}
...
void sdio_set_timing_by_freq(struct msdc_host *host, u32 clock)
{
	void __iomem *base = host->base;
	...
	if (clock >= 200000000) {
		...
		sdr_set_field(host->top + MSDC_TOP_DAT_TUNE_CTRL1,
			      MSDC_TOP_SDIO_DATA_TUNE_SEL, 1);
...
bool sdio_online_tune_fail = false;
...
static void msdc_set_mclk(struct msdc_host *host, unsigned char timing, u32 hz)
{
	...
	if (host->hw->host_function == MSDC_SDIO && hz >= 100 * 1000 * 1000) {
		if (support_sdio_data_tune()) {
			if (sdio_online_tune_fail)
				hz = 50 * 1000 * 1000;
		} else {
			hz = 50 * 1000 * 1000;
			sdio_set_timing_by_freq(host, hz);
		}
	}
	if (support_sdio_data_tune())
					      ...
					      unsigned long timeout)
{
	...
	if (intsts & cmdsts) {
		...
		if ((intsts & MSDC_INT_CMDRDY) || (intsts & MSDC_INT_ACMDRDY)
			...
			if (host->hw->host_function != MSDC_SDIO || !sdio_autok_processed)
			...
			cmd->error = (unsigned int)-ETIMEDOUT;
			if (host->card_selected && !sdio_autok_processed)
...
check_fifo1:
			...
			if ((msdc_rxfifocnt() >= MSDC_FIFO_THD) &&
				...
				if (ints & MSDC_INT_DATCRCERR) {
					if (host->hw->host_function != MSDC_SDIO || !sdio_autok_processed)
						ERR_MSG("[msdc%d] DAT CRC(0x%x), Left DAT: %d bytes\n",
							host->id, ints, left);
					data->error = (unsigned int)-EIO;
				} else if (ints & MSDC_INT_DATTMO) {
					if (host->hw->host_function != MSDC_SDIO || !sdio_autok_processed)
						ERR_MSG("[msdc%d] DAT TMO(0x%x), Left DAT: %d bytes\n",
							host->id, ints, left);
					data->error = (unsigned int)-ETIMEDOUT;
				} else {
...
#ifdef SDIO_USE_DATA_TUNE
#include "mt_vcore_dvfs.h"
#define AUTOK_CMD_TIMES			10
#define AUTOK_TUNING_INACCURACY		5
...
static u32 tune_latch_ck_cnt;
static bool dvfs_claimed_host;
...
static int autok_send_raw_cmd19(struct msdc_host *host, unsigned int fifo_clr)
{
	...
	if (fifo_clr)
		msdc_reset_hw(host->id);
	/*set timeout*/
	sdr_set_field(SDC_CFG, SDC_CFG_DTOC, 0);

	/* start command */
	while (readl(SDC_STS) & SDC_STS_SDCBUSY)
		cpu_relax();

	rawcmd =  (64 << 16)|(0 << 13)|(1 << 11)|(1 << 7)|(19);
	if (fifo_clr)
		writel(1, SDC_BLK_NUM);
	else
		writel(tune_latch_ck_cnt, SDC_BLK_NUM);
	writel(0, SDC_ARG);
	writel(rawcmd, SDC_CMD);

	/*Wait cmd done*/
	int_mask = MSDC_INT_CMDTMO | MSDC_INT_CMDRDY | MSDC_INT_RSPCRCERR;
	while (!(readl(MSDC_INT) & int_mask))
		cpu_relax();

	int_sts = readl(MSDC_INT);
	writel((int_mask & int_sts), MSDC_INT);
	...
	/*Wait Data done*/
	while ((readl(SDC_STS) & SDC_STS_SDCBUSY)) {
		fifo_cnt = readl(MSDC_FIFOCS) & MSDC_FIFOCS_RXCNT;
		sdr_set_field(MSDC_DBG_SEL, 0xffff, 0x0b);
		sdr_get_field(MSDC_DBG_OUT, 0x7ff, fifo_cnt);
		if (fifo_cnt >= 1024) {
			for (i = 0; i < 5; i++)
				readl(MSDC_RXDATA);
		}
...
/*
 *	if (ret)
 *		pr_err("[ERROR]%s error code :0x%x\n", __func__, ret);
 */
	return ret;
}
 ...
 *		-3 else	error
 **/
int sdio_autok_adjust_param(struct msdc_host *host, enum AUTOK_PARAM param,
		u32 *value, int rw)
{
	u32 *reg;
	u32 field;
...
static int autok_param_apply(struct msdc_host *host, u8 *autok_tune_res)
{
	...
	for (i = 0; i < TUNING_PARAM_COUNT; i++) {
		...
		sdio_autok_adjust_param(host, i, &value, AUTOK_WRITE);
	}
	return 0;
}
...
{
	sdio_autok_adjust_param(host, param, &value, AUTOK_WRITE);
	return 0;
}
...
static int autok_path_sel(struct msdc_host *host, bool need_autok)
{
	...
	if (need_autok) {
		...
		autok_write_param(host, WRDAT_CRCS_TA_CNTR,
				  host->hw->wdatcrctactr_sdr200);
		autok_write_param(host, INT_DAT_LATCH_CK,
				  host->hw->intdatlatcksel_sdr200);
		...
		/*data tune mode select*/
		sdr_write32(host->top + MSDC_TOP_DAT_TUNE_CTRL1, 0);
		sdr_write32(host->top + MSDC_TOP_DAT_TUNE_CTRL2, 0);
		sdr_write32(host->top + MSDC_TOP_DAT_TUNE_CTRL3, 0);
...
static int execute_online_tuning(struct	msdc_host *host)
{
	...
	do {
		...
		sdio_autok_adjust_param(host, CMD_RD_D_DLY, &uCmdPadDDly,
					AUTOK_WRITE);
		sdio_autok_adjust_param(host, CMD_EDGE, &uCmdEdge, AUTOK_WRITE);
		sdio_autok_adjust_param(host, CKGEN_MSDC_DLY_SEL, &uCkgenSel,
					AUTOK_WRITE);
		...
		if (autok_calc_bit_cnt(RawData) > 24) {
			...
			if (autok_check_scan_res(RawData, &RawScanRes))
				return -1;
	...
	do {
		...
		if (autok_calc_bit_cnt(RawData) > 24) {
			...
			if (autok_check_scan_res(RawData, &ScanRef))
				return -1;
	...
	do {
		...
		sdio_autok_adjust_param(host, CMD_RESP_RD_DLY, &uCmdIntDly,
					AUTOK_WRITE);
		sdio_autok_adjust_param(host, CMD_EDGE, &uCmdEdge, AUTOK_WRITE);
		sdio_autok_adjust_param(host, CKGEN_MSDC_DLY_SEL, &uCkgenSel,
					AUTOK_WRITE);
		...
		if (score < 8) { /*Pad Boundary occours*/
			...
			if (autok_check_scan_res(RawData, &RawScanPadRes))
				return -1;
	...
	*/
	...
	autok_tuning_parameter_init(host, p_autok_tune_res);
	if (uCmdEdge) {
		uHalfBound = g_autok_bound_info.bound_cycle / 2;
		if (uCmdIntDly < 16) {
			if ((uCmdIntDly + uHalfBound) > 31)
				uWCrcIntDly = 31;
			else
				uWCrcIntDly = uCmdIntDly + uHalfBound;
		} else {
			if (uCmdIntDly < uHalfBound)
				uWCrcIntDly = 0;
			else
				uWCrcIntDly = uCmdIntDly - uHalfBound;
		}
	...
	if ((RawData & 0xff) == 0xff) {
		pr_err("[AUTOK] use Default LatchCK: 0\r\n");
		uLatchCK  = 0;
	} else {
		for (i = 0; i < 8; i++) {
			if (RawData & (1 << i)) {
...
void apply_sdio_setting(struct msdc_host *host, u32 hz)
{
	...
	if (hz >= 200000000) {
		...
		/* After applying tuned parameters,
		   latck_ck may be not the default value */
		sdr_get_field(MSDC_PATCH_BIT0, MSDC_PB0_INT_DAT_LATCH_CK_SEL,
...
{
	if (!sdio_host || !support_sdio_data_tune())
		return 0;

	pr_err("[AUTOK] stop transfer. c_vol:%d, card:%d\n",
	       vcorefs_sdio_get_vcore_nml(), sdio_host->card_selected);
	if (!sdio_host->card_selected)
		return 0;
	mmc_claim_host(sdio_host->mmc);
	dvfs_claimed_host = true;
	return 0;
}
...
int sdio_start_ot_transfer(void)
{
	...
	pr_err("[AUTOK] start transfer. c_vol:%d, hz:%d, done:%d, card:%d\n",
	       vcorefs_sdio_get_vcore_nml(), sdio_host->mmc->ios.clock,
	       vol_autok_done, sdio_host->card_selected);

	if (!sdio_host->card_selected) {
		if (dvfs_claimed_host)
			mmc_release_host(sdio_host->mmc);
		dvfs_claimed_host = false;
		return 0;
	}

	if (vol_autok_done &&
	    sdio_host->mmc->ios.clock >= 200 * 1000 * 1000)
		apply_sdio_setting(sdio_host, sdio_host->mmc->ios.clock);
	if (dvfs_claimed_host)
		mmc_release_host(sdio_host->mmc);
	dvfs_claimed_host = false;
	return 0;
...
static void msdc_init_hw(struct msdc_host *host)
{
	...
	if (is_card_sdio(host)) {
		if (support_sdio_data_tune())
			apply_sdio_setting(host, 50 * 1000 * 1000);
		else
			sdio_set_timing_by_freq(host, 50 * 1000 * 1000);
	}
	if (host->id == 0) {
		sdr_get_field(EMMC50_PAD_DS_TUNE, MSDC_EMMC50_PAD_DS_TUNE_DLY1,
			host->saved_para.ds_dly1);
				...
				struct msdc_host *host)
{
	...
	if (pdev->id == 0) {
		host->bus_clk_ctrl = devm_clk_get(&pdev->dev, "MSDC0-CLOCK");
		...
		host->bus_clk_ctrl = devm_clk_get(&pdev->dev, "MSDC1-CLOCK");
	} else if (pdev->id == 2) {
		host->bus_clk_ctrl = devm_clk_get(&pdev->dev, "MSDC2-CLOCK");
	} else if (pdev->id == 3) {
		host->bus_clk_ctrl = devm_clk_get(&pdev->dev, "sdio-clock");
		if (IS_ERR(host->bus_clk_ctrl)) {
			pr_err("can not get msdc%d bus clock\n", pdev->id);
			ret = 1;
			goto out;
		}

		host->src_clk_ctrl = devm_clk_get(&pdev->dev, "sdio-hclk");
		if (IS_ERR(host->src_clk_ctrl)) {
			pr_err("can not get msdc%d hclk\n", pdev->id);
			ret = 1;
			goto out;
		}
		goto out;
...
int msdc_of_parse(struct mmc_host *mmc)
{
	...
	/* iomap register */
	host->top = of_iomap(np, 1);
	if (!host->top) {
		pr_err("can't of_top_iomap for msdc!!\n");
		return -ENOMEM;
	}
	pr_err("of_top_iomap for msdc @ 0x%p\n", host->base);
	...
	* property data isn't large enough.*/
	...
	//	pr_err("[MSDC%d] cd-gpios isn't found in DT\n", host->id);
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	if (strcmp(pdev->dev.of_node->name, "sdio") == 0) {
		...
		host->hw->cmdrtactr_sdr200 = 0x4;
		host->hw->wdatcrctactr_sdr200 = 0x2;
		host->hw->intdatlatcksel_sdr200 = 0x0;
		pr_err("platform_data hw:0x%p, is msdc3\n", host->hw);
	}
#endif
