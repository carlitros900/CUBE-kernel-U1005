...
struct msdc_host {
	...
	void __iomem *top_base;		/* top base address */
...
static const struct mt81xx_mmc_compatible mt8135_compat = {
	...
	.async_fifo = false,
	.data_tune = false,
	.compatible_name = "mediatek,mt8135-mmc",
};
...
static const struct mt81xx_mmc_compatible mt8163_compat = {
	...
	.async_fifo = true,
	.data_tune = true,
	.compatible_name = "mediatek,mt8163-mmc",
};
...
static const struct mt81xx_mmc_compatible mt8167_compat = {
	...
	.async_fifo = true,
	.data_tune = true,
	.compatible_name = "mediatek,mt8167-mmc",
};
...
static const struct mt81xx_mmc_compatible mt8173_compat = {
	...
	.async_fifo = false,
	.data_tune = false,
	.compatible_name = "mediatek,mt8173-mmc",
};
...
static const struct mt81xx_mmc_compatible mt2701_compat = {
	...
	.async_fifo = true,
	.data_tune = true,
	.compatible_name = "mediatek,mt2701-mmc",
};
...
static void msdc_set_mclk(struct msdc_host *host, unsigned char timing, u32 hz)
{
	...
	if (host->sclk <= 52000000) {
		...
		writel(host->def_tune_para.pad_cmd_tune, host->base + PAD_CMD_TUNE);
	} else {
...
static void msdc_init_hw(struct msdc_host *host)
{
	...
	host->def_tune_para.pad_cmd_tune = readl(host->base + PAD_CMD_TUNE);
	if (!strcmp(host->dev_comp->compatible_name, "mediatek,mt8173-mmc") &&
	    !is_e1_chip() && host->host_id == 1) {
		/* SD use data tune */
		top_sdr_set_bits(host->top_base, BIT(31)); /* SD select data tune */
...
/* this function is only for MT8173 E2 */
static int sd_tune_response(struct mmc_host *mmc, u32 opcode)
{
	...
	if (mmc->ios.timing == MMC_TIMING_UHS_SDR104) {
		/* set cmd ta to 4 */
		sdr_set_field(host->base + MSDC_PATCH_BIT1, MSDC_PATCH_BIT1_CMDTA, 4);
...
static int sd_tune_data(struct mmc_host *mmc, u32 opcode)
{
	...
	sdr_set_field(host->base + MSDC_PATCH_BIT, MSDC_INT_DAT_LATCH_CK_SEL, 1);
	if (mmc->ios.timing == MMC_TIMING_UHS_SDR104) {
		...
		sdr_set_field(host->base + MSDC_PATCH_BIT1, MSDC_PATCH_BIT1_WRTA, 3);
	...
	dev_err(host->dev, "edge: %d, data pad delay: 0x%x\n", edge, data_delay);
	return final_delay == 0xff ? -EIO : 0;
}
...
/* this function is only for MT8173 HS400 mode */
...
static int msdc_execute_tuning(struct mmc_host *mmc, u32 opcode)
{
	...
	pm_runtime_get_sync(host->dev);
	if (!strcmp(host->dev_comp->compatible_name, "mediatek,mt8173-mmc")) {
		/* MT8173 has special tune method */
		if (host->hs400_mode == true)
			ret = emmc_tune_response(mmc, opcode);
		else if (host->host_id == 0 || is_e1_chip())
			ret = msdc_tune_response(mmc, opcode);
		else
			ret = sd_tune_response(mmc, opcode);
		if (ret == -EIO) {
			dev_err(host->dev, "MT8173 Tune response fail!\n");
			goto out;
		}
		...
		if (host->hs400_mode == false) {
			...
			if (ret == -EIO) {
				dev_err(host->dev, "MT8173 Tune data fail!\n");
				goto out;
			}
		}
	} else if (host->dev_comp->data_tune && host->dev_comp->async_fifo) {
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	if (!strcmp(host->dev_comp->compatible_name, "mediatek,mt8173-mmc")) {
		...
		if (of_property_read_u32(pdev->dev.of_node, "host_id",
					 &host->host_id)) {
			dev_err(&pdev->dev, "Please add host_id at DTS!\n");
			ret = -EINVAL;
			goto host_free;
		}
	}
